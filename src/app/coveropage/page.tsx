  ...
  return (
    <header className="relative min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-slate-50 to-white dark:from-slate-900 dark:to-slate-800 px-6 md:px-16 overflow-hidden">
      ...
      <motion.div
        ...
        className="relative w-60 h-60 -mt-24 md:w-80 md:h-80 rounded-full overflow-hidden border-4 border-emerald-500 shadow-lg"
      >
        <Image
          src={portrait}
          alt={`${name} Portrait`}
      ...
      >
        {[ 
          ...
          <motion.a
            ...
            whileTap={{ scale: 0.9 }}
            className="text-slate-700 dark:text-slate-300 hover:text-emerald-600 transition"
          >
            {icon}
          </motion.a>
        ))}
